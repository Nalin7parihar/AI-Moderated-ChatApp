...
@router.put('/{id}',status_code=status.HTTP_202_ACCEPTED,response_model=UserRead)
...
@router.get('/search',status_code=status.HTTP_202_ACCEPTED,response_model=List[UserRead])
async def search_users(q:str,db : Session = Depends(get_session),offset : int =0,limit : int = 100,current_user : User = Depends(get_current_user)):
  ...
  statement = (select(User).where(or_(User.name.ilike(search_term),User.email.ilike(search_term))).offset(offset).limit(limit))
...
@router.get("/",response_model=list[UserReadWithAdminInfo],tags=["admin"])
...
@router.get("/{id}",response_model=UserReadWithAdminInfo,tags=["admin"])
...
@router.patch('/{id}',response_model=UserReadWithAdminInfo,tags=["admin"])
